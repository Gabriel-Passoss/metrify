...
describe('(E2E) Delete Store', () => {
  ...
  it('should be able to delete a store', async () => {
    ...
    const response = await request(app.server)
      .delete(`/organizations/${organization.slug}/stores/${store.slug}`)
  ...
  it('should not be able to delete a store without being an admin or manager', async () => {
    const owner = await makeUser()
    await prisma.user.create({
      data: owner,
    })
    ...
    expect(response.body).toMatchObject({
      message: "You're not allowed to delete a store.",
    })
  })
})
